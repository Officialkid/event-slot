...
export function WhatsAppFloatingButton({ contactNumber, contactMode, eventTitle, eventDate }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative flex items-center gap-2 text-white font-medium text-sm pl-3 pr-4 py-3 rounded-full shadow-lg transition-colors ${isCall ? "bg-[#C8F55A] hover:bg-[#b8e34f] text-[#0A0A0A]" : "bg-[#25D366] hover:bg-[#1fbe5a]"}`}>
        {isCall ? (

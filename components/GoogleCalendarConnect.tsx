...
export function GoogleCalendarConnect({ isConnected, variant = 'profile' }: Props) {
  ...
  if (variant === 'compact') {
    return isConnected ? (
      ...
      <button
        ...
        className="flex items-center gap-2 border border-[#2A2A2A] rounded-lg
                   px-3 py-1.5 text-[#A3A3A3] text-xs hover:border-[#4285F4]/50
  ...
  // Full profile card variant
  return (
    <div className="border border-[#2A2A2A] rounded-2xl p-5 bg-[#141414] space-y-4">

      {/* Header */}
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-[#4285F4]/10 border border-[#4285F4]/30
        ...
        <div>
          ...
          <p className="text-[#525252] text-xs">
            {isConnected
              ? 'Your events sync automatically'
              : 'Sync EventSlot events to your calendar'}
          </p>
        </div>
        {isConnected ? (
          <span className="ml-auto text-xs bg-[#22C55E]/10 text-[#22C55E]
                           border border-[#22C55E]/30 px-2.5 py-1 rounded-full
                           font-medium flex items-center gap-1">
            <CheckCircle className="w-3 h-3" /> Connected
          </span>
        ) : (
          <span className="ml-auto text-xs bg-[#525252]/20 text-[#A3A3A3]
                           border border-[#2A2A2A] px-2.5 py-1 rounded-full">
            Not connected
          </span>
        )}
      </div>

      {/* Benefits list — shown when disconnected */}
      {!isConnected && (
      ...
      {isConnected && (
        <div className="bg-[#22C55E]/5 border border-[#22C55E]/20 rounded-xl p-3 space-y-1.5">
          ...
          <p className="text-[#A3A3A3] text-xs">
            • New events you create → added to Google Calendar immediately
          </p>
          <p className="text-[#A3A3A3] text-xs">
            • Event date/time changes → your calendar updates within seconds
          </p>
          <p className="text-[#A3A3A3] text-xs">
            • Cancelled events → marked as cancelled in your calendar
          </p>
          <p className="text-[#A3A3A3] text-xs">
            • Google sends reminders 1 day + 1 hour before each event
          </p>
        </div>
      )}
      ...
      {isConnected ? (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 border
                     border-[#2A2A2A] rounded-xl py-2.5 text-[#525252] text-sm
      ...
      <p className="text-[#525252] text-xs text-center">
        EventSlot only creates and updates events it manages.
        We never read your other calendar events.
      </p>
    </div>
  );
}

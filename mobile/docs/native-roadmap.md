...
- Native shell shows workspace sync source, event count, last refresh time, and retry action for Android live API QA.
...
- Native Profile groups store compliance links for privacy, terms, account deletion, website, and tester support review.
...
- Native Profile now includes a reusable 10-state UI catalogue for empty, loading, error, no-internet, slow-network, no-search-results, permission-denied, session-expired, form-validation, and success flows.

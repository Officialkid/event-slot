...
export default function AssistantPage() {
  ...
  const greeting      = getGreeting(authSession?.user?.name)
  ...
  // Auto-creates a session on first send — no channel pre-selection screen
  async function sendMessage(text: string, isVoice = false) {
    ...
    if (!activeId || activeSession?.ended) {
      // Spin up a new session automatically
      const channel: Channel = isVoice ? "voice" : "text"
      ...
      sid = data.sessionId
      const label = trimmed.slice(0, 40)
      const newSess: ChatSession = {
        id: sid, label,
        messages: [{ role: "user", content: trimmed, timestamp: new Date(), isVoice }],
        channel, ended: false, startedAt: new Date(),
      }
      setSessions(prev => [newSess, ...prev])
      setActiveId(sid)
      setSidebarOpen(false)
      fetch(`/api/assistant/session/${sid}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label }),
      }).catch(() => {})
    } else {
      sid = activeId
      addMessage(sid, "user", trimmed, isVoice)
    }
  ...
  function startRename(id: string, currentLabel: string) {
    setMenuId(null); setRenamingId(id); setRenameValue(currentLabel)
  }
  ...
  // ── Hold-to-record: press = start, release = stop + auto-transcribe ────────
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (chunksRef.current.length > 0) {
          await transcribeAndSend(new Blob(chunksRef.current, { type: "audio/webm" }))
        }
      }
  ...
  async function transcribeAndSend(blob: Blob) {
    ...
    if (!res.ok) {
      const data = await res.json()
      setError(res.status === 402 ? data.message : "Could not process voice message. Please type instead.")
    ...
    <div className="flex bg-[#0A0A0A] relative overflow-hidden" style={{ height: "calc(100dvh - 4rem - 4.5rem)" }}>
      ...
      <aside className={`fixed md:relative top-0 left-0 h-full z-40 w-72 flex flex-col bg-[#111111] border-r border-[rgba(240,237,230,0.07)] transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="flex items-center justify-between p-4 border-b border-[rgba(240,237,230,0.07)]">
          ...
          <button onClick={() => setSidebarOpen(false)} className="md:hidden text-[rgba(240,237,230,0.4)] hover:text-[#F0EDE6] text-xl leading-none">&#x2715;</button>
        </div>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        {/* Greeting home screen or chat messages */}
        {!activeSession ? (
          <div className="flex-1 flex flex-col items-center justify-center px-6 pb-10">
            <h1
              className="text-[2rem] sm:text-[2.4rem] font-semibold text-[#F0EDE6] mb-2 text-center leading-tight"
              style={{ fontFamily: "var(--font-instrument-serif)" }}
            >
              {greeting}
            </h1>
            <p className="text-sm text-[rgba(240,237,230,0.35)]" style={{ fontFamily: "var(--font-dm-sans)" }}>
              What are you working on?
            </p>
            {error && (
              <div className="mt-5 text-center">
                <p className="text-[#EF4444] text-sm">{error}</p>
                <button onClick={() => setError(null)} className="mt-2 text-xs text-[rgba(240,237,230,0.4)] underline">Dismiss</button>
              </div>
            )}
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
            ...
            {activeSession.messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div
                  ...
                  style={{ fontFamily: "var(--font-dm-sans)" }}
                >
                  {msg.isVoice && <span className="block text-xs opacity-50 mb-1">🎙️ Voice message</span>}
                ...
                <div className="bg-[#1A1A1A] border border-[rgba(240,237,230,0.06)] rounded-2xl rounded-bl-sm px-4 py-3">
                  {transcribing ? (
                    ...
                    <div className="flex gap-1.5 items-center h-4">
                      {[0,1,2].map(j => (
                        <div key={j} className="w-1.5 h-1.5 rounded-full bg-[rgba(240,237,230,0.3)] animate-bounce" style={{ animationDelay: `${j * 0.15}s` }} />
                      ))}
                    </div>
                  )}
                </div>
            ...
            {activeSession.ended && (
              <div className="flex justify-center pt-2">
                <div className="flex items-center gap-3 rounded-2xl border border-[rgba(200,245,90,0.2)] bg-[rgba(200,245,90,0.05)] px-4 py-2.5">
                  <span className="text-xs text-[rgba(200,245,90,0.7)]" style={{ fontFamily: "var(--font-dm-sans)" }}>Session ended</span>
                  <span className="text-[rgba(240,237,230,0.2)]">&middot;</span>
        ...
        <div className="px-4 pb-4 pt-2 flex-shrink-0">
          {recording && (
            <div className="flex items-center gap-2 mb-2 px-1">
              ...
              <span className="text-xs text-red-400" style={{ fontFamily: "var(--font-dm-sans)" }}>Recording… release to send</span>
            </div>
          )}
          {transcribing && (
            <div className="flex items-center gap-2 mb-2 px-1">
              <span className="text-xs text-[rgba(240,237,230,0.4)] animate-pulse" style={{ fontFamily: "var(--font-dm-sans)" }}>Transcribing…</span>
            </div>
          )}
          <div className="flex items-end gap-2 bg-[#141414] border border-[rgba(240,237,230,0.08)] rounded-[1.5rem] px-3 py-2.5 focus-within:border-[rgba(240,237,230,0.18)] transition-colors">

            {/* + new chat shortcut */}
            <button
              onClick={() => { setActiveId(null); setSidebarOpen(false) }}
              title="New chat"
              className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 self-end mb-0.5 bg-[#1E1E1E] hover:bg-[#2A2A2A] border border-[rgba(240,237,230,0.1)] text-[rgba(240,237,230,0.5)] hover:text-[#F0EDE6] transition-colors"
            >
            ...
            <textarea
              ...
              rows={1}
              className="flex-1 bg-transparent text-[#F0EDE6] text-sm placeholder:text-[rgba(240,237,230,0.25)] focus:outline-none resize-none py-1.5 max-h-36 overflow-y-auto disabled:opacity-50"
            ...
            <button
              ...
              className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 self-end mb-0.5 transition-all select-none ${recording ? "bg-red-500 scale-110 shadow-lg shadow-red-500/30" : "bg-[#1E1E1E] hover:bg-[#2A2A2A] border border-[rgba(240,237,230,0.1)]"} disabled:opacity-40`}
            >
              {recording ? (
            ...
            <button
              ...
              className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 self-end mb-0.5 bg-[#C8F55A] text-black hover:bg-[#b8e040] disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
          ...
          <p className="text-center text-[0.63rem] text-[rgba(240,237,230,0.13)] mt-1.5" style={{ fontFamily: "var(--font-dm-sans)" }}>
            Hold mic to record · 5 free voice messages / month
          </p>
        </div>

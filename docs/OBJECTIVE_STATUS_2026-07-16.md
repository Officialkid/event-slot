...
| Light/dark theme | Theme choice should work across the system, not only the landing page. | In progress. Several dashboard/report/email surfaces have been converted to theme tokens. Theme helper behavior is now covered by regression tests for default dark mode, light mode application, dark mode application, and local persistence. Needs page-by-page visual audit. |
...
| Reports free for organizers | Authorized organizers/team/admins should generate/download reports without payment. | Implemented, deployed, and covered by regression tests for free preview/download access, assigned team access, and unauthorized blocking. Report purchase UI/routes are guarded during payment pause. |

...
| Light/dark theme | Theme choice should work across the system, not only the landing page. | In progress. Several dashboard/report/email surfaces have been converted to theme tokens. The core event management page now uses semantic tokens for cards, menus, dialogs, empty states, tabs, inputs, metadata, and loading states, with a regression guard against dark-only palette values. Theme helper behavior is covered for default dark mode, light mode application, dark mode application, and local persistence. Other remaining pages still need conversion and a page-by-page visual audit. |

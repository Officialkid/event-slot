...
| Light/dark theme | Theme choice should work across the system, not only the landing page. | In progress. The shared dashboard shell, dashboard home/admin summaries, event list/detail/email tools, profile, feedback, insights, notifications, team, and sidebar install surfaces now use semantic theme tokens for neutral text, borders, inputs, cards, dialogs, navigation, and empty/loading states. Regression guards cover the converted signed-in surfaces against dark-only neutral palette values, while theme helper tests cover default dark mode, light/dark application, and local persistence. Remaining public/secondary pages still need conversion and a page-by-page visual audit. |

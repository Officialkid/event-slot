...
| Light/dark theme | Theme choice should work across the system, not only the landing page. | In progress. The shared dashboard shell, dashboard home/admin summaries, event list/detail/email tools, profile, feedback, insights, notifications, team, and sidebar install surfaces use semantic theme tokens. Every current admin route now uses the shared semantic palette, including the dashboard, users, health, launch, and internal payment-test screens. The sidebar refreshes its name/photo from the saved profile instead of relying only on a stale session. Public Terms, Privacy, unauthorized, not-found, global-error, and cache-recovery routes also follow the active theme. Regression guards cover these converted surfaces, while theme helper tests cover default dark mode, light/dark application, and local persistence. Remaining attendee and secondary routes still need conversion and a page-by-page visual audit. |
